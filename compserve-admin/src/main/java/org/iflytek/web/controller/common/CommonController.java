...
import org.iflytek.framework.config.ServerConfig;
...
public class CommonController
{
    ...
    public AjaxResult uploadFile(MultipartFile file) throws Exception
    {
        try
        {
            ...
            log.info("开始上传文件到对象存储: {}", file.getOriginalFilename());
            String url = minioUtils.upload(file);
            log.info("对象存储上传成功, url: {}", url);
    ...
    public AjaxResult uploadFiles(List<MultipartFile> files) throws Exception
    {
        try
        {
            ...
            for (MultipartFile file : files)
            {
                ...
                log.info("开始批量上传文件到对象存储: {}", file.getOriginalFilename());
                String url = minioUtils.upload(file);
                log.info("批量上传成功, url: {}", url);
            ...
            ajax.put("previewUrls", StringUtils.join(new ArrayList<String>() {{
                for (String u : urls) { add(minioUtils.getPrivateUrl(u)); }
            }}, FILE_DELIMITER));
