package org.iflytek.common.utils.http;
...
public class UserAgentUtils
{
    ...
    private static final String UA_ANALYZER_CLASS = "nl.basjes.parse.useragent.UserAgentAnalyzer";
    private static final String UA_CLASS = "nl.basjes.parse.useragent.UserAgent";
    private static final String UA_AGENT_NAME_VERSION = "AgentNameVersion";
    private static final String UA_OS_NAME_VERSION = "OperatingSystemNameVersion";

    private static final Object userAgentAnalyzer;
    private static final java.lang.reflect.Method parseMethod;
    private static final java.lang.reflect.Method uaGetMethod;
    private static final java.lang.reflect.Method valueGetMethod;

    static
    {
        Object analyzer = null;
        java.lang.reflect.Method localParseMethod = null;
        java.lang.reflect.Method localUaGetMethod = null;
        java.lang.reflect.Method localValueGetMethod = null;
        try
        {
            Class<?> analyzerClass = Class.forName(UA_ANALYZER_CLASS);
            Class<?> userAgentClass = Class.forName(UA_CLASS);

            Object builder = analyzerClass.getMethod("newBuilder").invoke(null);
            builder.getClass().getMethod("hideMatcherLoadStats").invoke(builder);
            builder.getClass().getMethod("withCache", int.class).invoke(builder, 5000);
            builder.getClass().getMethod("showMinimalVersion").invoke(builder);
            builder.getClass().getMethod("withField", String.class).invoke(builder, UA_AGENT_NAME_VERSION);
            builder.getClass().getMethod("withField", String.class).invoke(builder, UA_OS_NAME_VERSION);
            analyzer = builder.getClass().getMethod("build").invoke(builder);

            localParseMethod = analyzerClass.getMethod("parse", String.class);
            localUaGetMethod = userAgentClass.getMethod("get", String.class);
            Class<?> agentFieldClass = Class.forName("nl.basjes.parse.useragent.AgentField");
            localValueGetMethod = agentFieldClass.getMethod("getValue");
        }
        catch (Throwable ignore)
        {
            analyzer = null;
            localParseMethod = null;
            localUaGetMethod = null;
            localValueGetMethod = null;
        }
        userAgentAnalyzer = analyzer;
        parseMethod = localParseMethod;
        uaGetMethod = localUaGetMethod;
        valueGetMethod = localValueGetMethod;
    }
    ...
    {
        String agentNameVersion = getAnalyzedValue(userAgent, UA_AGENT_NAME_VERSION);
    ...
    {
        String operatingSystemNameVersion = getAnalyzedValue(userAgent, UA_OS_NAME_VERSION);
    ...
    /**
     * 预热 User-Agent 解析器，避免首次真实请求承担初始化成本。
     */
    public static void warmUp()
    {
        ...
        getBrowser("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/137.0.0.0 Safari/537.36");
        getOperatingSystem("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/137.0.0.0 Safari/537.36");
    }

    private static String getAnalyzedValue(String userAgent, String fieldName)
    {
        if (userAgentAnalyzer == null || parseMethod == null || uaGetMethod == null || valueGetMethod == null || StringUtils.isBlank(userAgent))
        {
            return UNKNOWN;
        }
        try
        {
            Object parsed = parseMethod.invoke(userAgentAnalyzer, userAgent);
            Object field = uaGetMethod.invoke(parsed, fieldName);
            Object value = valueGetMethod.invoke(field);
